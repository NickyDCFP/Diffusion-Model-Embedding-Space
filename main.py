...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="No training, just extract the class embeddings to a .csv"
    )
    parser.add_argument(
        "--override-num-embeddings",
        type=int,
        default=None,
        help="Change the number of embeddings to be loaded. Only use with --save-embeddings-only"
    )
    ...
    if args.override_num_embeddings is not None:
        metadata.num_classes = args.override_num_embeddings
    ...
    if args.save_embeddings_only:
        label_weights = model.label_emb.weight.data.cpu()
        label_weights_df = pd.DataFrame(label_weights.numpy())
        label_weights_df.to_csv(os.path.join(args.save_dir, 'embeddings.csv'))
        return
