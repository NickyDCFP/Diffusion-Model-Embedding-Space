...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_classes is not None:
            if self.label_embed_dim is not None:
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.num_classes is not None:
            ...
            if self.label_embed_dim is not None:
                dim_change = th.tensor(
                    [[1, 0] * (emb.shape[1] // 2)] + [[0, 1] * (emb.shape[1] // 2)]
                ).to(device=timesteps.device, dtype=th.float32)
                emb = emb + self.label_emb(y).matmul(dim_change)
            else:
