...
def train_one_epoch(
    ...
    for step, (images, labels) in enumerate(dataloader):
        ...
        labels = torch.tensor([metadata.num_classes] * len(images), dtype=torch.int64).to(args.device)
        ...
        with torch.no_grad():
            model.module.label_emb.weight.data[:metadata.num_classes] = prev_label_emb
    ...
    with tqdm(total=math.ceil(N / (args.batch_size * num_processes))) as pbar:
        while num_samples < N:
            ...
            if args.class_cond:
                ...
                #     args.device
                # )
                y = torch.tensor([num_classes] * len(xT), dtype=torch.int64).to(args.device)
            else:
                y = None
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Dimensionality of class embeddings. Only use if also using --class-cond"
    )
    ...
    #only update label_emb
    # optimizer = torch.optim.AdamW(model.label_emb.parameters(), lr=args.lr)
    ...
    #resize label_emb
    prev_label_emb = model.label_emb.weight.data.clone()
    prev_label_emd_dim = prev_label_emb.shape[1]
    
    model.label_emb = torch.nn.Embedding(metadata.num_classes + 1, prev_label_emd_dim)
    
    with torch.no_grad():
        model.label_emb.weight.data[:metadata.num_classes] = prev_label_emb
        # model.label_emb.weight.data[metadata.num_classes] = prev_label_emb[metadata.num_classes-1]
    model.label_emb.weight.requires_grad = True
    ...
    for epoch in range(args.epochs):
        ...
        train_one_epoch(model, train_loader, diffusion, optimizer, logger, None, args, prev_label_emb, metadata, local_rank)
        if not epoch % 1:
            ...
            if local_rank == 0:
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-{args.diffusion_steps}_steps-{args.sampling_steps}-sampling_steps_invert_{args.target_class}_embed_dim_{args.label_embed_dim}.png",
                    ),
        ...
        if local_rank == 0:
            torch.save(
                ...
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-{args.sampling_steps}-sampling_steps_invert_{args.target_class}_embed_dim_{args.label_embed_dim}.pt",
                ),
            )
            torch.save(
                args.ema_dict,
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-{args.sampling_steps}-sampling_steps_ema_{args.ema_w}_invert_{args.target_class}_embed_dim_{args.label_embed_dim}.pt",
                ),
            )
