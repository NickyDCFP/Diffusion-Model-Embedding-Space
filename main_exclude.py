...
def main():
    ...
    parser.add_argument("--exclude", type=int, default=0, help="Class to exclude while training")
    ...
    excluded_indices = [i for i, (_, label) in enumerate(train_set) if label != args.exclude]

    # Create a subdataset with all classes except exclude
    excluded_dataset = Subset(train_set, excluded_indices)
    train_set = excluded_dataset
    ...
    for epoch in range(args.epochs):
        ...
        if not epoch % 1:
            ...
            if local_rank == 0:
                cv2.imwrite(
                    os.path.join(
                        args.save_dir,
                        f"{args.arch}_{args.dataset}-{args.sampling_steps}-sampling_steps_exclude_{args.exclude}_embed_dim_{args.label_embed_dim}.png",
                    ),
        ...
        if local_rank == 0:
            torch.save(
                ...
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-{args.sampling_steps}-sampling_steps_exclude_{args.exclude}_embed_dim_{args.label_embed_dim}.pt",
                ),
            )
            torch.save(
                args.ema_dict,
                os.path.join(
                    args.save_dir,
                    f"{args.arch}_{args.dataset}-epoch_{args.epochs}-{args.sampling_steps}-sampling_steps_exclude_{args.exclude}_embed_dim_{args.label_embed_dim}_ema_{args.ema_w}.pt",
                ),
            )
